...
import torchvision
from .base_dataset_bbox_kp import BaseDataset, check_filenames_in_zipdata, recalculate_box_kps_and_verify_if_valid  
...
def draw_box(img, boxes, keypoints=None):
    draw = ImageDraw.Draw(img)
    # Define a list of colors for the keypoints
    ...
    if keypoints is not None:
        for kp in keypoints:
            for p in kp:
                ...
                # Draw each keypoint as a small circle with the color
                if p[0] > 0 and p[1] > 0:  # Assuming non-positive values are invalid/missing keypoints
